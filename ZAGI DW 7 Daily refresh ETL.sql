...
INSERT INTO 'salestransaction' ('tid', 'customerid', 'storeid', 'tdate') VALUES ('321', '1-2-333', 'S1', '2025-03-25')
INSERT INTO 'soldvia' ('productid', 'tid', 'noofitems') VALUES ('2X4', '321', '3')
INSERT INTO 'soldvia' ('productid', 'tid', 'noofitems') VALUES ('3X1', '321', '6')
INSERT INTO 'rentaltransaction' ('tid', 'customerid', 'storeid', 'tdate') VALUES ('543', '3-4-555', 'S3', '2025-03-25')
...
SELECT rv.duration * p.productpricedaily AS RevAmount, rv.tid, 'rentals, weekly' AS TransactionType, rt.customerid AS CustomerID, rt.storeid AS StoreID, p.productID, rt.tdate AS FullDate
FROM mihuynh_ZAGImore.rentaltransaction rt, mihuynh_ZAGImore.rentalProducts p, mihuynh_ZAGImore.rentvia rv
...
SELECT i.tid, i.RevAmount, p.ProductKey, c.CustKey, cld.CalendarKey, s.StoreKey, i.TransactionType, NOW(), FALSE
FROM IntTable i, Product_Dimension p, Customer_Dimension c, Calendar_Dimension cld, Store_Dimension s
WHERE c.CustomerID = i.CustomerID AND p.ProductID = i.ProductID AND s.StoreID = i.StoreID AND UPPER(LEFT(p.ProductType, 1)) = UPPER(LEFT(i.RevenueType, 1)) AND i.FullDate = cld.FullDate
